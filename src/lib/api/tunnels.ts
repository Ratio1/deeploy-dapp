...
export async function getTunnels(): Promise<{
    ...
    const cloudflare_account_id = import.meta.env.VITE_CF_ACCOUNT_ID;
    const cloudflare_api_key = import.meta.env.VITE_CF_API_KEY;
...
export async function createTunnel(alias: string) {
    const cloudflare_account_id = import.meta.env.VITE_CF_ACCOUNT_ID;
    const cloudflare_zone_id = import.meta.env.VITE_CF_ZONE_ID;
    const cloudflare_api_key = import.meta.env.VITE_CF_API_KEY;
    const cloudflare_domain = import.meta.env.VITE_CF_DOMAIN;
